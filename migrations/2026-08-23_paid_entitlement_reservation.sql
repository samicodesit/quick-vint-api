...
-- Keep stored manual grants intact, but only honor custom limits while the
-- profile still has paid entitlement. Revoke direct client access because
-- this security-definer function accepts an arbitrary profile ID.
...
BEGIN
  ...
  IF profile_row.subscription_status IN ('active', 'trialing', 'past_due', 'canceling')
    AND profile_row.custom_daily_limit IS NOT NULL
...
REVOKE ALL ON FUNCTION reserve_generation_request(
  uuid,
  text,
  text,
  integer,
...
GRANT EXECUTE ON FUNCTION reserve_generation_request(
  uuid,
  text,
  text,
  integer,
