...
export class VintedCountryDetector {
      ...
      for (const lang of languages) {
        if (lang.includes('-')) {
          ...
          if (this.VINTED_DOMAINS[countryCode]) {
            console.log('Detected by language preference:', countryCode, 'from', lang);
            return countryCode;
          }
        }
      }
  ...
  static getVintedUrl(countryCode) {
    const domain = this.VINTED_DOMAINS[countryCode] || this.VINTED_DOMAINS.DEFAULT;
  ...
  static getVintedUrlForUserSync() {
    const country = this.detectByTimezone() || 
